...
type UserInfoForCreate struct {
	Id         string  `json:"userid"`               // 必须;  员工UserID。对应管理端的帐号，企业内必须唯一
	Name       string  `json:"name"`                 // 必须;  成员名称。长度为0~64个字符
	...
	Mobile     string  `json:"mobile,omitempty"`     // 非必须; 手机号码。企业内必须唯一，更新后的成员mobile/weixinid/email三者不能同时为空
	Gender     int     `json:"gender,omitempty"`     // 非必须; 性别。gender=0表示男，=1表示女。默认gender=0
	Tel        string  `json:"tel,omitempty"`        // 非必须; 办公电话。长度为0~64个字符。必须企业内唯一
